...
class Puzzle:
    ...
    def process(self):
        ...
        start_node.fval = self.heuristic(start_node.data, goal)
        ...
        while True:
            ...
            for i in cur.data:
                for j in i:
                    print(j, end=" ")
                print("")
            """ If the difference between current node and goal node is 0 we have reached the goal node"""
            if self.heuristic(cur.data, goal) == 0:
                break
            for i in cur.generate_child():
                i.fval = self.heuristic(i.data, goal)
                self.open.append(i)
    ...
    def heuristic(self, start, goal):
        """ Heuristic function to calculate hueristic value"""
        temp = 0
